...
import TripController from '../../../src/controllers/TripController';
...
describe('trip controller', function() {
  ...
  it(
    'getTripsWithPagination With Existing Trips Should Return Trips Json',
    getTripsWithPagination_WithExistingTrips_ShouldReturnTripsJson,
  );
});
...
const tripPageArrayExpected = {
  trips: [TripExpected1, TripExpected2, TripExpected3] as ITripDTO[],
  totalPageCount: 1 as number,
};

//Implement the tests
//#region getTripsWithPagination
async function getTripsWithPagination_WithExistingTrips_ShouldReturnTripsJson() {
  // Arrange
  ...
  const ctrl = new TripController(tripServiceInstance as ITripService);

  // Act
  await ctrl.getTripsWithPagination(<Request>req, <Response>res, <NextFunction>next);

  // Assert
  sinon.assert.calledWith(res.json, sinon.match(tripPageArrayExpected));
}
